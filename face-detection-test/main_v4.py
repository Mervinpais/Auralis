...
def compare_face(embedding, db, threshold=0.3):
...
async def main():
    ...
    while True:
        ...
        if cmd == "exit":
            ...
            print("Interpreting frame")
            try:
                # DeepFace wants RGB uint8 arrays, ensure it's correct
                frame_rgb = np.array(frame, dtype=np.uint8)
                ...
                for face in faces:
                    ...
                    if known_match:
                        print(f"Recognized: {known_match}")
                        await speak(known_match)
                    else:
                        print("Unknown face")
                        await speak("Unknown face")
