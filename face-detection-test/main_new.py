...
# --- Save new face with multiple samples ---
def save_new_face(face_imgs, name):
    timestamp = datetime.now().strftime("%Y_%m_%d.%H_%M_%S")
    embeddings = []
    for idx, face_img in enumerate(face_imgs):
        face_file = os.path.join(KNOWN_FACES_DIR, f"{name}_{timestamp}_{idx}.jpg")
        npy_file = os.path.join(EMBEDDINGS_DIR, f"{name}_{timestamp}_{idx}.npy")
        cv2.imwrite(face_file, face_img)
        embedding = get_face_embedding(face_img)
        np.save(npy_file, embedding)
        embeddings.append(embedding)
    speak(f"Saved new face {name}")
    return embeddings
